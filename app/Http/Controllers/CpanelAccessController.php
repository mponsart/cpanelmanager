...
class CpanelAccessController extends Controller
{
    ...
    public function index()
    {
        ...
        // If a password is already configured in .env, no need to expose a public key
        // to the browser – the manual login will use the server-side password directly.
        if (! empty(config('cpanel.password'))) {
            return view('cpanel.index', array_merge(compact('host', 'port', 'username'), ['publicKey' => null, 'hasConfiguredPassword' => true]));
        }
        ...
        return view('cpanel.index', array_merge(compact('host', 'port', 'username', 'publicKey'), ['hasConfiguredPassword' => false]));
    }
    ...
    public function manualLogin(Request $request): RedirectResponse
    {
        ...
        // If a password is configured server-side, use it directly without
        // requiring any browser-side input or RSA decryption.
        ...
        // No configured password – fall back to the RSA-encrypted browser input.
