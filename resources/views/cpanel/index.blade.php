...
@php
    $rows = [
        ['label' => 'Hôte',        'value' => $host,     'sensitive' => false, 'icon' => '<svg width="15" height="15" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5"><rect x="1" y="4" width="14" height="8" rx="1.5"/><circle cx="4" cy="8" r="1" fill="currentColor" stroke="none"/><line x1="7" y1="6" x2="7" y2="10"/></svg>'],
        ['label' => 'Port',         'value' => $port,     'sensitive' => false, 'icon' => '<svg width="15" height="15" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5"><circle cx="8" cy="8" r="6.5"/><path d="M8 4v4l3 2"/></svg>'],
        ['label' => 'Utilisateur',  'value' => $username, 'sensitive' => false, 'icon' => '<svg width="15" height="15" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5"><circle cx="8" cy="5" r="3"/><path d="M2 14c0-3 2.7-5 6-5s6 2 6 5"/></svg>'],
        ['label' => 'Domaine',      'value' => $domain,   'sensitive' => false, 'icon' => '<svg width="15" height="15" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5"><circle cx="8" cy="8" r="6.5"/><ellipse cx="8" cy="8" rx="3" ry="6.5"/><line x1="1.5" y1="8" x2="14.5" y2="8"/></svg>'],
        ['label' => 'Token API',    'value' => $token,    'sensitive' => true,  'icon' => '<svg width="15" height="15" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M10 1l5 5-7 7-5-5z"/><circle cx="11.5" cy="4.5" r="1" fill="currentColor" stroke="none"/><path d="M6 10L1 15"/></svg>'],
        ['label' => 'Mot de passe', 'value' => $password, 'sensitive' => true,  'icon' => '<svg width="15" height="15" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5"><rect x="3" y="7" width="10" height="7" rx="1.5"/><path d="M5 7V5a3 3 0 116 0v2"/></svg>'],
    ];
...
@if($cpanelUrl)
<div class="card mb-4">
    <div class="card-title">Connexion rapide</div>
    <p class="text-muted" style="font-size:.875rem;margin-bottom:16px;">
        Connectez-vous directement à l'interface cPanel via une session temporaire sécurisée ou par identifiants.
    </p>
    <div style="display:flex;gap:10px;flex-wrap:wrap;align-items:center;">
        <form action="{{ route('cpanel.connect') }}" method="POST" style="display:inline;">
            @csrf
        ...
        <form action="{{ route('cpanel.manual-login') }}" method="POST" style="display:inline;">
            ...
            <button type="submit" class="btn btn-ghost">
                ...
                Connexion manuelle (mot de passe)
            </button>
        </form>
    </div>
</div>
            ...
            <tr style="border-top:1px solid var(--border);">
                <td class="text-muted" style="width:180px;padding:12px 10px;vertical-align:middle;font-size:.875rem;">
                    <span style="display:inline-flex;align-items:center;gap:8px;">
                        <span style="opacity:.5;display:inline-flex;">{!! $row['icon'] !!}</span>
                        {{ $row['label'] }}
                    </span>
                </td>
